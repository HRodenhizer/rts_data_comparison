...
plot_individual_prediction <- function(imagery_type, raster_data, vector_data, dims) {
  ...
  figure <- ggplot(data = vector) +
    ...
    scale_fill_manual(breaks = levels(input_data$color_hex),
                      ...
                      guide = 'none') +
    new_scale('fill') +
    geom_sf(aes(geometry = inverse_mask_geometry,
                fill = 'Masked\nArea',
                alpha = 'Masked\nArea'),
            color = 'transparent') +
    scale_fill_manual(name = 'Masked\nArea',
                      values = 'black',
                      drop = FALSE,
                      guide = guide_legend(
                        order = 3,
    ...
    scale_alpha_manual(name = 'Masked\nArea',
                       values = 0.5,
                       drop = FALSE,
                       guide = guide_legend(
                         order = 3,
    ...
    scale_linewidth_manual(name = 'Other\nRTS',
                           values = c(0.25),
                           guide = guide_legend(
                             order = 4,
                             label = FALSE
    ...
    geom_sf(aes(geometry = rts_geometry,
                ...
                linewidth = 'RTS of\nInterest'),
            fill = 'transparent') +
    scale_color_manual(name = 'RTS of\nInterest',
                       values = c('gray90'),
                       drop = FALSE,
                       guide = guide_legend(
                         order = 2,
    ...
    scale_linewidth_manual(name = 'RTS of\nInterest',
                           values = c(0.5),
                           guide = guide_legend(
                             order = 2,
                             label = FALSE
...
plot_pred_rgb_2 <- function(pid, imagery_types, dims) {
  ...
  current_polys <- polys %>%
    filter(id == pid) %>%
    select(imagery, id, status, rts_iou, pq_ci, pq_ci_size, bbox_geometry, 
           inverse_mask_geometry, rts_geometry, val_geometry, pred_geometry) %>%
    mutate(status = factor(status, levels = c('Active', 'General')))
...
pred_as_poly <- function(prediction, threshold = 0.5) {
  ...
  poly <- st_as_sf(as.polygons(layer, values = TRUE)) %>%
    rename(tk_class = prediction, pred_bg_geometry = geometry) %>%
    mutate(tk_class = case_when(tk_class == 0 ~ 'pred_bg',
                                tk_class == 1 ~ 'pred'),
           id = get_rast_id(prediction)) %>%
    pivot_wider(names_from = 'tk_class',
                values_from = 'pred_bg_geometry',
...
val_as_poly <- function(prediction) {
  ...
  poly <- st_as_sf(as.polygons(layer, values = TRUE)) %>%
    rename(tk_class = validation, val_bg_geometry = geometry) %>%
    mutate(tk_class = case_when(tk_class == 0 ~ 'val_bg',
                                tk_class == 1 ~ 'val'),
           id = current_id) %>%
    pivot_wider(names_from = 'tk_class',
                values_from = 'val_bg_geometry',
                names_glue = '{tk_class}_geometry') %>%
...
polys <- polys %>%
  left_join(masks %>%
            ...
            by = c('imagery', 'id')) %>%
  mutate(
    pred_geometry = st_as_sfc(
      pmap(list(pred_geometry, 
                mask_geometry), 
           ~ st_intersection(...)),
      crs = crs(polys)
    ),
    pred_bg_geometry = st_as_sfc(
      pmap(list(pred_bg_geometry, 
                mask_geometry), 
           ~ st_intersection(...)),
      crs = crs(polys)
    ),
    val_mask_geometry = st_as_sfc(
      pmap(list(val_geometry, 
                mask_geometry), 
           ~ st_intersection(...)),
      crs = crs(polys)
    ),
    val_bg_mask_geometry = st_as_sfc(
      pmap(list(val_bg_geometry, 
                mask_geometry), 
           ~ st_intersection(...)),
      crs = crs(polys)
    ),
    inverse_mask_geometry = st_as_sfc(
      pmap(list(bbox_geometry, 
                mask_geometry), 
           ~ st_difference(...)),
      crs = crs(polys)
    ),
...
```{r}
iou_anova = lm(rts_iou ~ imagery, data = polys)
summary(iou_anova)
emmeans(iou_anova, specs = pairwise~imagery)
```
...
ggplot(
  ...
  scale_fill_manual(name = '',
                    ...
                    guide = guide_legend(order = 2)) +
  new_scale('fill') +
  geom_histogram(aes(fill = imagery),
                 binwidth = 0.05) +
  scale_fill_manual(name = 'Undetected RTS',
  ...
  geom_vline(
    data = polys %>%
      st_drop_geometry() %>%
      summarise(rts_area_median = median(rts_area)/10000,
                by = imagery),
    aes(xintercept = rts_area_median, 
        linetype = 'All RTS\nMedian')
    ) +
  geom_vline(
    data = polys %>%
      st_drop_geometry() %>%
      summarise(rts_area_mean = mean(rts_area)/10000,
                by = imagery),
    aes(xintercept = rts_area_mean, 
        linetype = 'All RTS\nMean')
    ) +
  scale_linetype_manual(name = '',
  ...
  scale_x_continuous(
    name = 'RTS Area (ha)',
    limits = c(0, 
               mean(filter(polys, imagery == 'WorldView')$rts_area)/10000),
...
# ggsave('./figures/undetected_rts_histogram.pdf',
#        height = 4.5,
#        width = 4.5)
# ggsave('./figures/undetected_rts_histogram.jpg',
#        height = 4.5,
#        width = 4.5)
...
polys %>%
  filter(rts_iou == 0) %>%
  st_drop_geometry() %>%
  reframe(percent_less_than_0.1 = ecdf(rts_area/10000)(0.1), 
          percent_less_than_0.15 = ecdf(rts_area/10000)(0.15), 
          .by = imagery)
...
# ggsave('./figures/area_iou_for_GA.jpg',
#        height = 4,
#        width = 6)
```
...
ggplot(zonal_stats_mean %>%
  ...
  scale_y_continuous(name = expression(Delta~' z-score (RTS—BG)')) +
  scale_color_manual(name = "Prediction\nQuality",
                     values = pq_colors) +
    facet_grid(. ~ imagery,
               scales = 'free_y') +
...
current_dims <- current_polys %>%
  ...
  bind_cols(map_dfr(.$rts_geometry, 
                    ~ st_bbox(.x))) %>%
  mutate(x_min = (xmin + xmax)/2 - map_dimensions$x_spread_max/2,
         x_max = (xmin + xmax)/2 + map_dimensions$x_spread_max/2,
         y_min = (ymin + ymax)/2 - map_dimensions$y_spread_max/2,
         y_max = (ymin + ymax)/2 + map_dimensions$y_spread_max/2) %>%
  select(x_min, x_max, y_min, y_max)
...
for (idx in 1:3) {
  ...
  imagery_type = imagery_labels[idx]
  
  rgb <- current_rast %>%
    filter(imagery == imagery_type)
  
  vector <- current_polys %>%
    filter(imagery == imagery_type)
  label <- paste('IoU:', round(vector$rts_iou, 2))
  
  figures[[idx]] <- ggplot(data = vector) +
    geom_raster(data = rgb,
                aes(x = x, y = y, fill = color_hex),
                inherit.aes = FALSE) +
    scale_fill_manual(breaks = levels(input_data$color_hex),
    ...
    new_scale('color') +
    geom_sf(aes(geometry = pred_geometry, color = 'Prediction'),
            fill = 'transparent',
            linewidth = 0.5) +
    scale_color_manual(name = 'Prediction', values = 'black') +
    geom_label(x = current_dims$x_min,
               y = current_dims$y_min,
               label = label,
               size = 2.5,
               hjust = 0,
               vjust = 0,
               ...
               label.r = unit(0, 'lines'),
               label.size = unit(0, 'lines')) +
    coord_sf(xlim = c(current_dims$x_min, current_dims$x_max),
             ylim = c(current_dims$y_min, current_dims$y_max),
             expand = FALSE) +
    facet_grid(imagery ~ .) +
    theme(axis.title = element_blank(),
          axis.text = element_blank(),
...
# ggsave('./figures/prediction_example_vertical.jpg',
#        figure,
#        height = 4,
#        width = 3)
```
...
## Imagery Comparison Figure

```{r}
imagery_example_1 <- ggplot(data = input_data %>%
                              filter(id == id_subset[2]) |>
                              mutate(x = x - mean(x),
                                     y = y - mean(y),
                                     .by = imagery)) +
  geom_raster(aes(x = x, y = y, fill = color_hex)) +
  scale_fill_manual(breaks = levels(input_data$color_hex),
                    values = levels(input_data$color_hex),
                    guide = 'none') +
  facet_grid(. ~ imagery) +
...
imagery_example_2 <- ggplot(data = input_data %>%
                              filter(id == id_subset[3]) |>
                              mutate(x = x - mean(x),
                                     y = y - mean(y),
                                     .by = imagery)) +
...
imagery_example_2b <- ggplot(data = input_data %>%
                              filter(id == id_subset[3]) |>
                              mutate(x = x - mean(x),
                                     y = y - mean(y),
                                     .by = imagery)) +
...
# ggsave('figures/imagery_example_1.jpg',
#        imagery_example_1,
#        height = 1.7,
#        width = 10)
# ggsave('figures/imagery_example_2_vertical.jpg',
#        imagery_example_2b,
#        height = 4,
#        width = 2)
# ggsave('figures/imagery_example_2.jpg',
#        imagery_example_2,
#        height = 3,
#        width = 10)
```

## Imagery Layers Figure

```{r}
example_input_data <- maxar_input[id_subset[3]][[1]]

# RGB (from input_data tibble)
example_input_rgb <- ggplot(
  data = input_data %>%
    filter(id == id_subset[3] & imagery == 'WorldView') |>
    mutate(x = x - mean(x),
           y = y - mean(y))
) +
  geom_raster(aes(x = x, y = y, fill = color_hex)) +
...
# NIR
nir <- example_input_data %>%
    subset('SENT_NIR') %>%
    as.data.frame(xy = TRUE)

example_input_nir <- ggplot(nir) +
  geom_raster(aes(x = x, y = y, fill = SENT_NIR)) +
  scale_fill_viridis(name = '',
                     breaks = c(
                       min(nir$nir, na.rm = TRUE),
                       max(nir$nir, na.rm = TRUE)
                     ),
                     labels = c('Low', 'High')) +
  theme(axis.title = element_blank(),
        axis.text = element_blank(),
...
# NDVI
ndvi <- example_input_data %>%
    subset('ndvi') %>%
    as.data.frame(xy = TRUE)

example_input_ndvi <- ggplot(ndvi) +
  geom_raster(aes(x = x, y = y, fill = ndvi)) +
  scale_fill_viridis(name = '',
                     breaks = c(
                       min(ndvi$ndvi, na.rm = TRUE),
                       max(ndvi$ndvi, na.rm = TRUE)
                       ),
                     labels = c('Low', 'High')) +
  theme(axis.title = element_blank(),
        axis.text = element_blank(),
...
# NDWI
ndwi <- example_input_data %>%
    subset('ndwi') %>%
    as.data.frame(xy = TRUE)

example_input_ndwi <- ggplot(ndwi) +
  geom_raster(aes(x = x, y = y, fill = ndwi)) +
  scale_fill_viridis(name = '',
                     breaks = c(
                       min(ndwi$ndwi, na.rm = TRUE),
                       max(ndwi$ndwi, na.rm = TRUE)
                     ),
                     labels = c('Low', 'High')) +
  theme(axis.title = element_blank(),
        axis.text = element_blank(),
...
# Relative Elevation
elev <- example_input_data %>%
    subset('relative_elevation') %>%
    as.data.frame(xy = TRUE)

example_input_elev <- ggplot(elev) +
  geom_raster(aes(x = x, y = y, fill = relative_elevation)) +
  scale_fill_viridis(name = '',
                     breaks = c(
                       min(elev$relative_elevation, na.rm = TRUE),
                       max(elev$relative_elevation, na.rm = TRUE)
                     ),
                     labels = c('Low', 'High')) +
  theme(axis.title = element_blank(),
        axis.text = element_blank(),
        axis.ticks = element_blank()) +
  coord_fixed(expand = FALSE) +
  facet_grid(. ~ 'Relative Elevation')

# example_input_elev

# Shaded Relief
sr <- example_input_data %>%
    subset('shaded_relief') %>%
    as.data.frame(xy = TRUE)

example_input_sr <- ggplot(sr) +
  geom_raster(aes(x = x, y = y, fill = shaded_relief)) +
  scale_fill_viridis(name = '',
                     breaks = c(
                       min(sr$shaded_relief, na.rm = TRUE),
                       max(sr$shaded_relief, na.rm = TRUE)
                     ),
                     labels = c('Low', 'High')) +
  theme(axis.title = element_blank(),
        axis.text = element_blank(),
        axis.ticks = element_blank()) +
  coord_fixed(expand = FALSE) +
  facet_grid(. ~ 'Shaded Relief')

# example_input_sr

# RTS Label
label <- example_input_data %>%
    subset('RTSlabel') %>%
    as.data.frame(xy = TRUE, na.rm = FALSE) %>%
  mutate(
    RTSlabel = case_when(
      RTSlabel == 1 ~ factor('RTS', 
                             levels = c('RTS', 'Background')),
      is.na(RTSlabel) ~ factor('Background', 
                               levels = c('RTS', 'Background')))
  )

example_input_label <- ggplot(label) +
  geom_tile(
    aes(x = x, y = y, fill = RTSlabel), 
    color = 'black'
    ) +
  scale_fill_manual(name = 'RTS Label',
  ...
  theme(axis.title = element_blank(),
        ...
        axis.ticks = element_blank()) +
  coord_fixed(expand = FALSE) +
  facet_grid(. ~ 'RTS Label')

example_input_label
```

```{r}
input_layers_example <- example_input_rgb + 
  example_input_nir +
  example_input_ndvi + 
  example_input_ndwi +
  example_input_elev + 
...
input_layers_example

# ggsave('figures/input_layers_example.jpg',
#        input_layers_example,
#        height = 11,
#        width = 6.5)
```
